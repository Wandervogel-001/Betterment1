...
class AIModelSelectionView(View):
    def __init__(self, db, original_interaction, currently_active_model=None):
        super().__init__(timeout=300)
        self.db = db
        self.original_interaction = original_interaction
        self.currently_active_model = currently_active_model
    ...
    def _build_category_embed(self):
        ...
        embed.add_field(name="Currently Active", value=f"```\n{self.currently_active_model}\n```", inline=False)
        provider_list = "\n".join(MODEL_MAP.keys())
